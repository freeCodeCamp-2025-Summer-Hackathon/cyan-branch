...
export async function createBoxAction(formData) {
  try {
    ...
    // Revalidate the page path to reflect the new box
...
export async function generateLinkAction(formData) {
  try {
    ...
    const hours = Number.parseInt(formData.get("hours") || "24");
    ...
    const { getBoxById } = await import("@/prisma/queries");
    ...
    const ttlMs = hours * 60 * 60 * 1000; // Convert hours to milliseconds
    const { generateLink } = await import("@/utils/linkUtils");

    const link = await generateLink({ ttlMs, boxId });
...
export async function toggleLinkStatusAction(token) {
  try {
    ...
    const { getLinkByToken, updateLinkStatus } = await import("@/utils/linkUtils");
...
export async function deleteLinkAction(token) {
  try {
    ...
    const { getLinkByToken, deleteLink } = await import("@/utils/linkUtils");
    const link = await getLinkByToken(token);
...
export async function extendLinkExpirationAction(token, additionalHours) {
  try {
    const session = await getServerSession(authOptions);

    if (!session?.user?.id) {
      throw new Error("Unauthorized: Must be logged in to extend links");
    }

    const { getLinkByToken, extendLinkExpiration } = await import("@/utils/linkUtils");
    const link = await getLinkByToken(token);
    ...
    if (!box || box.adminId !== session.user.id) {
      throw new Error("Unauthorized: You do not have permission to extend this link");
    }

    const additionalMs = additionalHours * 60 * 60 * 1000;
    const extendedLink = await extendLinkExpiration(token, additionalMs);

    revalidatePath(`/dashboard/box/${link.boxId}`);
    return { success: true, data: extendedLink };
  }
  catch (error) {
    console.error("Extend link expiration action error:", error);
    return { success: false, error: error.message };
  }
}
